...
const PLAN_SCHEMA = {
  ...
  required: ['setName', 'vibe', 'genre', 'bpmTarget', 'targetBpmRange', 'arcShape', 'phases', 'energyArc', 'crateStrategy', 'opener'],
  properties: {
    setName: { type: 'string' },
    vibe: { type: 'string' },
    ...
    arcShape: { type: 'string', enum: ['journey', 'peak-time', 'warmup', 'cooldown'], description: 'Overall shape of the set energy curve' },
    phases: {
      type: 'array',
      minItems: 4,
      maxItems: 8,
      description: 'Ordered phases forming a real energy curve with tension/release and micro-peaks, not a flat ramp. Each phase ~= one track.',
      items: {
        type: 'object',
        additionalProperties: false,
        required: ['name', 'energy', 'targetEnergy', 'intent'],
        properties: {
          name: { type: 'string', enum: PHASES },
          energy: { type: 'number', description: '0..1 energy of this phase' },
          targetEnergy: { type: 'number', description: '0..1 energy the next track should reach (micro-peaks/valleys live here)' },
          intent: { type: 'string' },
        },
      },
    },
...
const CRATE_SYSTEM =
  'You are the Set Director / crate orchestrator for AI DJ. Your job is only to choose real songs and steer the energy arc. ' +
  ...
  '- HARMONIC MIXING: prefer Camelot-compatible keys (same code, ±1 number, or relative major/minor A/B swap). A deliberate bigger key jump is allowed ONLY as an intentional energy lift into a build/peak - never randomly.\n' +
  '- TEMPO LANE: a real set holds a tight BPM lane. Stay within a few BPM of the running lane; half/double-time is fine for lane math, but do not jump lanes (e.g. 95 -> 170) except as a deliberate reset at a finale/bridge. Genre conventions: house ~120-128, techno ~125-140, disco/funk ~110-120, hip-hop/trap ~85-100 (or 140-150 half-time), amapiano ~110-115, afrobeats ~100-118, reggaeton ~90-100, dnb ~170-175, Bollywood/desi pop ~95-115. Treat these as guidance, not hard rules.\n' +
  '- ENERGY ARC: a set is a WAVE, not a flat ramp. Build tension and release it: warmup -> build -> peak -> brief breathe -> second peak -> finale, with micro-peaks and valleys. Use the per-track targetEnergy to place those peaks/valleys deliberately.\n' +
...
const DJ_ARTIST_SYSTEM =
  'You are the AI DJ Artist agent. A separate Set Director already picked the incoming song. Your only job is live performance: beat/phrase alignment, tempo ride, faders, EQ, bass, filters, echo, reverbWash, vinylBrake, and loop rolls.\n' +
  ...
  (DJ_ARTIST_RULES ? `\n\nMandatory AI DJ Artist skill rules:\n${DJ_ARTIST_RULES}` : '');

function loadDjArtistRules() {
  try {
    return readFileSync(path.join(__dirname, '..', 'skills', 'ai-dj-artist', 'references', 'dj-performance-rules.md'), 'utf8').slice(0, 18000);
  } catch {
    return '';
  }
}
...
export async function planSet({ vibe, genre, bpmTarget }, apiKey) {
  ...
  const user =
    ...
    ' the energy arc, and a one-line crate strategy.' +
    ' Design `phases` as a real energy CURVE (4-8 phases, one per track) with tension and release - include at least one breathe/valley between peaks and set each phase\'s targetEnergy to place micro-peaks deliberately. Pick an arcShape (journey | peak-time | warmup | cooldown) that fits the scene.';
  const raw = await callTool(apiKey, PLANNER_MODEL, CRATE_SYSTEM, user, 'return_plan', PLAN_SCHEMA, 1300);
  return normalizePlan(raw, { vibe, genre: genre || scene, scene, bpmTarget });
}

export async function nextMix({ current, setPhase, bpmTarget, laneBpm, arcTarget, arcDirection, genre, played = [], memory = {} }, apiKey) {
  const c = current || {};
  const scene = str(genre, 'the set');
  const mem = memoryBrief(memory);
  const lane = Math.round(Number(laneBpm) || Number(bpmTarget) || DEFAULT_BPM);
  const user =
    `Continue the autonomous DJ set. Scene/crate: "${scene}". Running tempo LANE ~${lane} BPM - keep the next pick in this lane (the engine can ride a few BPM, but do NOT jump lanes). Phase: ${oneOf(setPhase, PHASES, 'build')}.\n` +
    (Number.isFinite(Number(arcTarget)) ? `SET ARC: aim the next track's energy toward ${num(arcTarget, 0.6, 0, 1).toFixed(2)} (${str(arcDirection, 'hold')} from here) - this is the planned wave, place a peak or a breather accordingly.\n` : '') +
    ...
    (mem ? `SET MEMORY: ${mem}\n` : '') +
    'Pick the next REAL, in-scene song (harmonic + in-lane tempo) that hits the arc energy target. ' +
...
export async function designTransition({ outgoing, incoming, setPhase, bpmTarget, memory = {} }, apiKey) {
  ...
  const user =
    ...
    `phrase ${num(n.phraseConfidence, 0.4, 0, 1).toFixed(2)}, downbeat ${num(n.downbeatConfidence, 0.4, 0, 1).toFixed(2)}, vocal density ${num(n.vocalDensity, 0.35, 0, 1).toFixed(2)}, mixability ${num(n.mixabilityScore, 0.5, 0, 1).toFixed(2)}; mixes in cleanly around ${Math.round(num(n.mixInSec, 0, 0, 6000))}s, main drop ~${Math.round(num(n.dropSec, 0, 0, 6000))}s, ${Math.round(num(n.phraseBars, 16, 1, 64))}-bar phrasing. Entry windows: ${windowBrief(n.bestEntryWindows)}.\n` +
    ...
    'PHRASE/DROP ALIGNMENT: time the blend so the incoming drop lands on an outgoing phrase boundary (the engine cues the incoming so its drop hits on the one) - never drop a new track over a dying vocal. ' +
...
export function normalizePlan(raw = {}, defaults = {}) {
  ...
  return {
    setName: str(raw.setName, 'AI DJ Live Session'),
    vibe: str(raw.vibe, defaults.vibe || scene),
    genre,
    bpmTarget: bpm,
    targetBpmRange: { min: Math.min(min, max), max: Math.max(min, max) },
    arcShape: oneOf(raw.arcShape, ['journey', 'peak-time', 'warmup', 'cooldown'], 'journey'),
...
function normalizePhases(phases) {
  const fallback = [
    { name: 'warmup', energy: 0.35, targetEnergy: 0.45, intent: 'Establish groove and tempo.' },
    { name: 'build', energy: 0.55, targetEnergy: 0.66, intent: 'Raise momentum with harmonic picks.' },
    { name: 'build', energy: 0.68, targetEnergy: 0.74, intent: 'Tighten the lane, add a micro-peak.' },
    { name: 'peak', energy: 0.86, targetEnergy: 0.9, intent: 'Deliver the biggest records.' },
    { name: 'release', energy: 0.6, targetEnergy: 0.55, intent: 'Breathe with a groovier track.' },
    { name: 'peak', energy: 0.82, targetEnergy: 0.88, intent: 'Second peak.' },
    { name: 'finale', energy: 0.7, targetEnergy: 0.7, intent: 'Land the set with a signature record.' },
  ];
  if (!Array.isArray(phases)) return fallback;
  const out = phases
    .map((p) => {
      const energy = num(p?.energy, 0.5, 0, 1);
      return {
        name: oneOf(p?.name, PHASES, 'build'),
        energy,
        targetEnergy: num(p?.targetEnergy, energy, 0, 1),
...
export async function generatePerformanceScript({ analysis, meta }, apiKey) {
  ...
  const phraseBars = Math.round(num(analysis?.phraseBars, 16, 1, 64));
  const dropBeat = analysis?.dropSec ? Math.round(Number(analysis.dropSec) * (bpm / 60)) : 0;
  const sectionsBrief = Array.isArray(analysis?.sections)
    ? analysis.sections.slice(0, 8).map((s) => `${s.kind}@${Math.round((Number(s.startSec) || 0) * (bpm / 60))}b`).join(', ')
    : '';

  const user = `Generate a live performance script for the track "${meta?.title || 'Unknown'}" by ${meta?.artist || 'Unknown'}.
  The track is ${Math.round(duration)} seconds long (~${totalBeats} beats) at ${Math.round(bpm)} BPM, ${phraseBars}-bar phrasing.
  Energy: ${Number(analysis?.energy ?? 0.5).toFixed(2)}/1.0.${dropBeat ? ` Main drop ~beat ${dropBeat}.` : ''}${sectionsBrief ? ` Sections: ${sectionsBrief}.` : ''}

  Perform PRO and TASTEFUL - like a real DJ working the track, never busy or gimmicky:
  - Build a filterSweep or flangerSwoosh riser in the ~2 bars BEFORE a drop or the start of a new phrase to build tension.
  - Use echoThrow on vocal tails / at the end of a phrase (every ${phraseBars * 2}-${phraseBars * 4} beats), not mid-phrase.
  - Use bassCut sparingly right before a drop for impact; reverbWash only for epic/spacey breakdowns.
  - Space events out (a few across the whole track). Align every event to a phrase or bar boundary. Do NOT stack effects.
  Return 3 to 6 events sorted by beat.`;

  const system = 'You are the AI DJ Artist performing this track live. You ride EQs and filters in time with the phrasing to hype the crowd WITHOUT ruining the music - tasteful, phrase-aligned, never busy.';
